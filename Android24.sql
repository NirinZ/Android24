...
`TransformationName` VARCHAR(100) NOT NULL UNIQUE,
`TransformationShortcut` VARCHAR(16) NOT NULL UNIQUE,
...
`AttackName` VARCHAR(100) NOT NULL UNIQUE,
`AttackShortcut` VARCHAR(16) NOT NULL UNIQUE,
`AttackPowerUp` INT UNSIGNED NOT NULL DEFAULT 1,
`DefencePowerUp` INT UNSIGNED NOT NULL DEFAULT 1,
`SpeedPowerUp` INT UNSIGNED NOT NULL DEFAULT 1,
...
ALTER TABLE `users_transformations` ADD CONSTRAINT `users_transformations_TransformationShortcut_transformations_TransformationShortcut` FOREIGN KEY (`TransformationShortcut`) REFERENCES `transformations`(`TransformationShortcut`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `users_attacks` ADD CONSTRAINT `users_attacks_AttackShortcut_attacks_AttackShortcut` FOREIGN KEY (`AttackShortcut`) REFERENCES `attacks`(`AttackShortcut`) ON DELETE CASCADE ON UPDATE CASCADE;
